...
local function checkmessage (prog, msg, debug)
  ...
  if not m then error("no error for prog: " .. prog, 2) end
  assert(string.find(m, msg, 1, true),
    "expected '" .. msg .. "' in: " .. tostring(m) .. "\n  prog: " .. prog)
end
...
-- Skip: requires 5.4.6/5.4.7 debug info improvements for field names
...
-- Skip: go-lua doesn't include metamethod name in error messages
-- checkmessage([[
...
-- Skip: go-lua doesn't include upvalue names in error messages
-- checkmessage("local a,b,c; (function () a = b+1.1 end)()", "upvalue 'b'")
assert(not doit"local aaa={bbb={ddd=next}}; aaa.bbb:ddd(nil)")

-- Skip: upvalues being indexed do not go to the stack
-- checkmessage("local a,b,cc; (function () a = cc[1] end)()", "upvalue 'cc'")
-- checkmessage("local a,b,cc; (function () a.x = 1 end)()", "upvalue 'a'")

-- Skip: go-lua doesn't report variable names with custom _ENV
-- checkmessage("local _ENV = {x={}}; a = a + 1", "global 'a'")

-- Skip: go-lua doesn't include variable names in arithmetic error messages
...
if false then  -- debug.upvalueid not supported
_G.D = debug
checkmessage([[
  -- create light udata
...
if false then  -- string.dump not supported
do
...
t = nil
-- Skip: go-lua doesn't include variable names in arithmetic/index error messages
...
-- Skip: go-lua reports field '__gc' instead of 'no value' (different debug info)
...
local function lineerror (s, l)
  ...
  if line ~= l and not (not line and not l) then
    error("lineerror FAIL: expected line " .. tostring(l) .. " got " .. tostring(line) .. " msg: " .. tostring(msg), 2)
  end
end
...
-- testing syntax limits (commented out: nesting limits differ in Go implementation)
--[[
